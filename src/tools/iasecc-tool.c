...
update_token_label(int card_type, char *sopin, char *new_token_label)
{
    ...
    if (card_type != CARD_TYPE_OBERTHUR_IASECC)   {
	    fprintf(stderr, "Only Oberthur flavor of IAS/ECC can be used. Card type %d\n", card_type);
		return -1;
    }
    ...
    for (ii=0, asn1_total_len = 0; ii < (unsigned)(*(data + offs) & 0x1F); ii++)
        asn1_total_len = asn1_total_len * 0x100 + *(data + offs + ii + 1);
    offs += (*(data + offs) & 0x1F) + 1;

    if (*(data + offs) != SC_ASN1_INTEGER)   {
	    fprintf(stderr, "Parse TokenInfo: expected SC_ASN1_INTEGER, have %02X\n", *(data + offs));
        goto err;
    }
    offs++;
    offs += *(data + offs) + 1;

    while ((*(data + offs) == SC_ASN1_UTF8STRING) || (*(data + offs) == SC_ASN1_TAG_OCTET_STRING))   {
        if (verbose)
            printf("%s +%i: waiting label tag ... current tag is 0x%02X\n", __FILE__, __LINE__, *(data + offs));
        offs++;
        offs += *(data + offs) + 1;
    }

    if (*(data + offs) != SC_ASN1_TAG_CONTEXT)   {
	    fprintf(stderr, "Parse TokenInfo: expected SC_ASN1_TAG_CONTEXT, have %02X\n", *(data + offs));
        goto err;
    }
    label_len = *(data + offs + 1);
...
int main(int argc, char *argv[])
{
	...
	if (r != SC_SUCCESS) {
		fprintf(stderr, "Failed to bind PKCS#15 card: %s\n", sc_strerror(r));
		return 1;
	}
